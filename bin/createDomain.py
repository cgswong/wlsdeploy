...
create('MyAD', 'weblogic.security.providers.authentication.ActiveDirectoryAuthenticator', 'AuthenticationProvider');

# change the order of the authentication provider (only works in online mode)
#try:
#	set('AuthenticationProviders',jarray.array([ObjectName('Security:Name=myrealmMyAD'), ObjectName('Security:Name=myrealmDefaultAuthenticator'), ObjectName('Security:Name=myrealmDefaultIdentityAsserter')], ObjectName));
...
set('GroupBaseDN', 'dc=ds,dc=mydomain,dc=com');
